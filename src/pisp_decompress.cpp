 ...
 * pisp_decompress.cpp
 *
 * Implementation of PISP_COMP1 (8-bit compressed RAW) decompressor.
 * Ported from libcamera source code (pisp_decompress.cpp / lut.h)
 */

#include "pisp_decompress.hpp"
#include "pisp_decompress_lut.hpp"

// Define the Lookup Table (LUT)
// This table maps the 8-bit compressed values to their 10-bit equivalent.
const uint16_t lut_combined_decompress[256] = {
...
/**
 * @brief Decompresses PISP_COMP1 data.
 * The logic is a simple 1:1 lookup from the 8-bit value to the 16-bit value.
 */
                     ...
                     size_t height)
{
    ...
    for (size_t i = 0; i < num_pixels; ++i) {
        // Get the 8-bit compressed value
        uint8_t compressed_value = input_buffer[i];
        
        // Use the LUT to find the 10-bit (in 16-bit container) decompressed value
        output_buffer[i] = lut_combined_decompress[compressed_value];
    }
}
